...
?>
        ...
        ?>
            <div class="wrap">
                ...
                <?php
                // Show success message if account was disconnected
                if (isset($_GET['disconnected']) && 'true' === $_GET['disconnected']) {
                    echo '<div class="notice notice-success is-dismissible"><p>' . __('Google Drive account has been disconnected successfully.', 'akashic-forms') . '</p></div>';
                }
                // Show error message if OAuth token exchange failed
                if (isset($_GET['auth_error']) && 'token_exchange_failed' === $_GET['auth_error']) {
                    echo '<div class="notice notice-error is-dismissible"><p>' . __('Google Drive authorization failed. The token exchange was unsuccessful. Please try again.', 'akashic-forms') . '</p></div>';
                }
                ?>
                ...
                if (!empty($client_id) && !empty($client_secret)) {
                    ...
                    if ($google_client && !$google_client->getAccessToken()) {
                        ...
                        if ( 'no_refresh_token' === $token_status['status'] ) {
                            $notice_class = 'notice-error';
                        } elseif ( 'expired' === $token_status['status'] ) {
                            $notice_class = 'notice-warning';
                        }

                        echo '<div class="notice ' . $notice_class . ' inline" style="margin: 20px 0; padding: 10px;">';
                        echo '<p><strong>' . esc_html( $token_status['message'] ) . '</strong></p>';
                        if ( ! empty( $token_status['expires'] ) ) {
                            echo '<p>' . sprintf( __( 'Token expires: %s', 'akashic-forms' ), esc_html( $token_status['expires'] ) ) . '</p>';
                        }
                        echo '</div>';

                        $disconnect_url = wp_nonce_url(
                            add_query_arg('akashic_forms_google_disconnect', '1', admin_url('admin.php?page=akashic-forms-google-drive-settings')),
                            'akashic_forms_google_disconnect'
                        );
                        echo '<p>';
                        echo '<a href="' . esc_url($disconnect_url) . '" class="button button-secondary" onclick="return confirm(\'' . esc_js(__('Are you sure you want to disconnect Google Drive? You will need to re-authorize to connect again.', 'akashic-forms')) . '\');">';
                        echo '<span class="dashicons dashicons-dismiss" style="margin-top: 4px;"></span> ' . __('Disconnect Google Account', 'akashic-forms');
                        echo '</a>';
...
<?php
        ...
        public function handle_google_drive_disconnect()
        {
            if (isset($_GET['akashic_forms_google_disconnect']) && '1' === $_GET['akashic_forms_google_disconnect']) {
                ...
                check_admin_referer('akashic_forms_google_disconnect');

                // Delete the access token to disconnect
                delete_option('akashic_forms_google_access_token');

                // Redirect back to settings page with success message
                wp_safe_redirect(add_query_arg(array('page' => 'akashic-forms-google-drive-settings', 'disconnected' => 'true'), admin_url('admin.php')));
                exit;
            }
        }
